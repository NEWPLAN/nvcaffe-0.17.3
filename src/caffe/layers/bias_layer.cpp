...
void BiasLayer<Ftype, Btype>::Forward_cpu(const vector<Blob *> &bottom,
                                          const vector<Blob *> &top)
{
  const Ftype *bias_data = ((bottom.size() > 1) ? bottom[1] : this->blobs_[0].get())->template cpu_data<Ftype>();
                                           ...
                                           const vector<bool> &propagate_down, const vector<Blob *> &bottom)
{
  ...
  {
    const Btype *top_diff = top[0]->cpu_diff<Btype>();
    Btype *bias_diff = (bias_param ? this->blobs_[0].get() : bottom[1])->template mutable_cpu_diff<Btype>();

...
void Net::Init(const NetParameter &in_param)
{
  ...
  for (int layer_id = 0; layer_id < param.layer_size(); ++layer_id)
  {
    ...
    bool share_from_root = !inner_net_ && !Caffe::root_solver() && root_net_->layers_[layer_id]->ShareInParallel();
    ...
    {
      mutable_layer_param->mutable_convolution_param()->set_conv_algos_override(param.default_conv_algos_override());
    }
                    ...
                    set<string> *available_blobs, map<string, int> *blob_name_to_idx)
{
  ...
  else
  {
    ...
    Type ftype = layer_param.has_forward_type() ? layer_param.forward_type() : param.default_forward_type();
    Type btype = layer_param.has_backward_type() ? layer_param.backward_type() : param.default_backward_type();
...
void Net::CopyTrainedLayersFrom(const NetParameter &param)
{
  ...
  for (int i = 0; i < num_source_layers; ++i)
  {
    ...
    if (source_layer_type == "BatchNorm")
    {
      ...
      {
        const float scale_factor = target_blobs[2]->cpu_data<float>()[0] == 0.F ? 0.F : 1.F / target_blobs[2]->cpu_data<float>()[0];
      ...
      if (!layers_[i]->skip_apply_update(j))
      {
        ...
        if (param_owners_[lip] < 0)
        {
          ...
          if (learnable_params_[param_id]->diff_type() == t)
          {
            ...
            void *p = learnable_params_[param_id]->current_mutable_data_memory(Caffe::mode() == Caffe::GPU);
            (void)p;
          }
        }
      }

...
template <typename Ftype, typename Btype>
void DataLayer<Ftype, Btype>::init_offsets()
{
...
template <typename Ftype, typename Btype>
void DataLayer<Ftype, Btype>::InitializePrefetch()
{
...
template <typename Ftype, typename Btype>
void DataLayer<Ftype, Btype>::DataLayerSetUp(const vector<Blob *> &bottom, const vector<Blob *> &top)
{
...
bool DataLayer<Ftype, Btype>::load_batch(Batch *batch, int thread_id, size_t queue_id)
{
  ...
  const int color_mode = this->transform_param_.force_color() ? 1 : (this->transform_param_.force_gray() ? -1 : 0);
  ...
  for (size_t entry = 0; entry < batch_size; ++entry)
  {
    ...
    if (use_gpu_transform)
    {
      ...
      else
      {
        ...
        src_ptr = datum->data().size() > 0 ? &datum->data().front() : reinterpret_cast<const char *>(&datum->float_data().Get(0));

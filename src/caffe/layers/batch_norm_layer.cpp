...
void BatchNormLayer<Ftype, Btype>::LayerSetUp(const vector<Blob *> &bottom, const vector<Blob *> &top)
{
  ...
  if (param.has_scale_filler() || param.has_bias_filler())
  { // implicit set
    scale_bias_ = true;
  }
...
template <typename Ftype, typename Btype>
void BatchNormLayer<Ftype, Btype>::Forward_cpu(const vector<Blob *> &bottom, const vector<Blob *> &top)
{
  int N = bottom[0]->shape(0);
  int C = channels_;

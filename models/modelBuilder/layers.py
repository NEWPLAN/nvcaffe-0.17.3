...
def addConvRelu(model, name, bottom, num_output,
    ...
    model, top = addActivation(
        model=model, name="{}/relu".format(name), bottom=top, type="ReLU")
    return model, top
...
def addBnRelu(model, name, bottom):
    ...
    model, top = addBN(model, name="{}/bn".format(name),
                       bottom=bottom, top="{}bn".format(name))
    model, top = addActivation(
        model, name="{}/relu".format(name), bottom=top, type="ReLU")
    return model, top
...
def addConvElu(model, name, bottom, num_output,
    ...
    model, top = addConv(model=model, name=name, bottom=bottom, num_output=num_output,
                         ...
                         residual=residual, residual_init=residual_init)
    model, top = addActivation(
        model=model, name="{}/elu".format(name), bottom=top, type="ELU")
    return model, top

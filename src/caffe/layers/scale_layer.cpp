                                            ...
                                            const vector<bool> &propagate_down, const vector<Blob *> &bottom)
{
      ...
      (scale_param && this->param_propagate_down_[0]))
  {
    ...
    Btype *product = (is_eltwise ? scale->mutable_cpu_diff<Btype>() : (in_place ? temp_->template mutable_cpu_data<Btype>() : bottom[0]->mutable_cpu_diff<Btype>()));
    ...
    if (!is_eltwise)
    {
      ...
      {
        const Btype *sum_mult = sum_multiplier_.cpu_data();
        sum_result = (outer_dim_ == 1) ? scale->mutable_cpu_diff<Btype>() : sum_result_.mutable_cpu_data();

...
BatchTransformer<Ftype, Btype>::BatchTransformer(int target_device, size_t rank_,
                                                 size_t queues_num, const TransformationParameter &transform_param, bool gpu_transform) : InternalThread(target_device, rank_, 1, false),
...
void BatchTransformer<Ftype, Btype>::resize(bool skip_to_next)
{
  ...
  for (size_t i = size; i < queues_num_; ++i)
  {
    ...
    shared_ptr<Batch> batch = gpu_transform_ ? make_shared<Batch>(tp<Ftype>(), tp<Ftype>()) : make_shared<Batch>(tp<Ftype>(), tp<Btype>());

...
def showResults(img_file, results, labelmap=None, threshold=None, display=None):
    ...
    for res in results:
        ...
        if 'score' in res:
            ...
            display_text = name
        ax.text(bbox[0], bbox[1], display_text, bbox={
...
if __name__ == "__main__":
    ...
    with open(result_file, "r") as f:
        for line in f.readlines():
            ...
            result["bbox"] = [float(xmin), float(
                ymin), float(xmax), float(ymax)]
            if save_dir:
                out_file = "{}/{}.png".format(save_dir,
    ...
    for img_file, results in img_results.iteritems():
        showResults(img_file, results, labelmap,
                    visualize_threshold, display_classes)

      ...
      for (int j = 0; j < match_index.size(); ++j)
      {
        ...
        if (bp_inside)
        {
          ...
          loc_pred_data[count * 4] =
              (match_bbox.xmin() < 0 || match_bbox.xmin() > 1) ? gt_encode.xmin() : loc_pred[j].xmin();
          loc_pred_data[count * 4 + 1] =
              (match_bbox.ymin() < 0 || match_bbox.ymin() > 1) ? gt_encode.ymin() : loc_pred[j].ymin();
          loc_pred_data[count * 4 + 2] =
              (match_bbox.xmax() < 0 || match_bbox.xmax() > 1) ? gt_encode.xmax() : loc_pred[j].xmax();
          loc_pred_data[count * 4 + 3] =
              (match_bbox.ymax() < 0 || match_bbox.ymax() > 1) ? gt_encode.ymax() : loc_pred[j].ymax();
        }
        else
        {
    ...
    if (all_gt_bboxes.find(i) != all_gt_bboxes.end())
    {
           ...
           it != match_indices.end(); ++it)
      {
        ...
        for (int j = 0; j < num_priors; ++j)
        {
          ...
          const int gt_label = map_object_to_agnostic ? background_label_id + 1 : all_gt_bboxes.find(i)->second[match_index[j]].label();
                      ...
                      const int top_k, vector<pair<Dtype, int>> *score_index_vec)
{
  ...
  // Sort the score pair according to the scores in descending order
  std::partial_sort(
      score_index_vec->begin(), score_index_vec->begin() + std::min(score_index_vec->size(), (size_t)top_k),
...
template void GetMaxScoreIndex(const float16 *scores, const int num,
                               const float threshold, const int top_k,
...
template void ApplyNMSFast(const float *bboxes, const float *scores, const int num,
                           const float score_threshold, const float nms_threshold,
                           const float eta, const int top_k, vector<int> *indices);
template void ApplyNMSFast(const double *bboxes, const double *scores, const int num,
                           const float score_threshold, const float nms_threshold,
                           const float eta, const int top_k, vector<int> *indices);
template void ApplyNMSFast(const float16 *bboxes, const float16 *scores, const int num,
                           const float score_threshold, const float nms_threshold,
                           const float eta, const int top_k, vector<int> *indices);

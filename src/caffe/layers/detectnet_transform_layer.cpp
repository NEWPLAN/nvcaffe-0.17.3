...
DetectNetTransformationLayer<Dtype>::DetectNetTransformationLayer(
    const LayerParameter &param) : Layer<Dtype, Dtype>(param),
...
DetectNetTransformationLayer<Dtype>::blobToMats(
    const Blob &images) const
{
  CHECK_EQ(images.channels(), 3);
  vector<Mat3v> result;
  result.reserve(images.num());

  for (size_t iImage = 0; iImage != images.num(); ++iImage)
  {
    const Dtype *image_data = &images.cpu_data<Dtype>()[images.offset(iImage, 0, 0, 0)];
...
DetectNetTransformationLayer<Dtype>::blobToLabels(
    const Blob &labels) const
{
  vector<vector<BboxLabel>> result;
  result.reserve(labels.num());

  for (size_t iLabel = 0; iLabel != labels.num(); ++iLabel)
  {
    const Dtype *source = &labels.cpu_data<Dtype>()[labels.offset(iLabel, 0, 0, 0)];
    ...
    Blob *_dest) const
{
  ...
  {
    Dtype *destination = &_dest->mutable_cpu_data<Dtype>()[_dest->offset(iImage, 0, 0, 0)];
    ...
    const vector<Blob *> &top)
{
  ...
  for (size_t iImage = 0; iImage != inputImages.size(); ++iImage)
  {
    ...
    Dtype *outputLabel = &outputLabels.mutable_cpu_data<Dtype>()[outputLabels.offset(iImage, 0, 0, 0)];
    ...
    cv::Size original_size)
{
  ...
  { // Randomly enable some augmentations

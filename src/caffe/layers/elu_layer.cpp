                                         ...
                                         const vector<Blob *> &top)
{
  ...
  {
    top_data[i] = lambda * (std::max(bottom_data[i], Ftype(0.)) + alpha * (exp(std::min(bottom_data[i], Ftype(0.))) - 1.F));
  }
}
                                          ...
                                          const vector<Blob *> &bottom)
{
  if (propagate_down[0])
  {
    ...
    for (int i = 0; i < count; ++i)
    {
      bottom_diff[i] = lambda * (top_diff[i] * ((bottom_data[i] > 0) + (alpha + top_data[i]) * (bottom_data[i] <= 0)));
    }
  }
}

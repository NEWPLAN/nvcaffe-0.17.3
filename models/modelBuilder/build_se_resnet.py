...
def addSEBlock(model, name, bottom, num_output, reduction=1):
    ...
    model, top = addPool(model=model, name='{}.global_pool'.format(
        name), bottom=bottom, pool_type="AVE", kernel_size=0, stride=1, global_pooling=True)

    model, top = addConvRelu(model=model, name='{}.conv_down'.format(
        name), bottom=top, num_output=num_output//reduction, kernel_size=1)
    model, top = addConv(model=model, name='{}.conv_up'.format(
        name), bottom=top, num_output=num_output,  kernel_size=1)
    model, top = addSigmoid(
        model=model, name='{}.prob'.format(name), bottom=top)
    return model, top
...
def addSERes(model, name, bottom, num_output, group, j, fix_dim, dilation=False):
    ...
    block, se_top = addSEBlock(model=block, name='{}se'.format(
        prefix), bottom=top, num_output=(num_output*4), reduction=16)
    ...
    block, top = addAxpy(model=block, name='{}axpy'.format(
        prefix), bottom_1=se_top, bottom_2=top, bottom_3=res_top)

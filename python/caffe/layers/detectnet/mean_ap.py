...
def score_det(gt_bbox_list, det_bbox_list):
    ...
    for k in range(gt_bbox_list.shape[0]):
        ...
        cur_det_bbox = np.asarray(
            list(filter(lambda a: a.tolist() != [0, 0, 0, 0], cur_det_bbox)))

        # Remove  zeros from label bboxes
        cur_gt_bbox = gt_bbox_list[k, :, 0:4]
        cur_gt_bbox = np.asarray(
            list(filter(lambda a: a.tolist() != [0, 0, 0, 0], cur_gt_bbox)))
        ...
        tp = np.asarray([np.append(j, 1)
                         for j in cur_det_bbox[np.where(det_matched == 1)]])
        fp = np.asarray([np.append(j, 2)
                         for j in cur_det_bbox[np.where(det_matched == 0)]])
        tn = np.asarray([np.append(j, 3)
                         for j in cur_gt_bbox[np.where(gt_matched == 0)]])
...
def calcmAP(scored_detections, self):
    ...
    self.precision = divide_zero_is_zero(
        self.true_positives, self.true_positives+self.false_positives)*100.00
    self.recall = divide_zero_is_zero(
        self.true_positives, self.true_positives+self.true_negatives)*100.00

                                           ...
                                           const vector<Blob *> &top)
{
  ...
  for (int i = 0; i < count; ++i)
  {
    ...
    top_data[i] = std::max(bottom_data[i], Ftype(0)) + slope_data[c] * std::min(bottom_data[i], Ftype(0));
  }
}
                                            ...
                                            const vector<Blob *> &bottom)
{
  ...
  if (propagate_down[0])
  {
    ...
    {
      int c = (i / dim) % channels / div_factor;
      bottom_diff[i] = top_diff[i] * ((bottom_data[i] > 0) + slope_data[c] * (bottom_data[i] <= 0));
    }
  }
}

...
{
  if (content_size > 1 && content[0] == 255 && content[1] == 216)
  { // probably jpeg
  ...
  const int flag = color_mode < 0 ? cv::IMREAD_GRAYSCALE : (color_mode > 0 ? cv::IMREAD_COLOR : cv::IMREAD_ANYCOLOR);
  ...
  for (int i = 0; i < map.item_size(); ++i)
  {
    ...
    if (strict_check)
    {
      if (!label_to_display_name->insert(
                                    std::make_pair(label, display_name))
               .second)
      {
        LOG(FATAL) << "There are many duplicates of label: " << label;
        return false;

...
def get_test_accuracy(log, top_k):
    ...
    accuracy = re.findall(
        r'Test net output #\d: accuracy/top-{top_k} = (\d*.\d*)'.format(top_k=top_k), log)
    if len(accuracy) == 0:
        accuracy = re.findall(
            r'Test net output #\d: top-{top_k} = (\d*.\d*)'.format(top_k=top_k), log)
    if len(accuracy) == 0:
        accuracy = re.findall(
            r'Test net output #\d: loss/top-{top_k} = (\d*.\d*)'.format(top_k=top_k), log)
    if len(accuracy) == 0:
        accuracy = re.findall(
            r'Test net output #\d: accuracy/top{top_k} = (\d*.\d*)'.format(top_k=top_k), log)
    if len(accuracy) == 0:
        accuracy = re.findall(
            r'Test net output #\d: accuracy = (\d*.\d*)', log)
...
def get_test_loss(log):
    ...
    if len(loss) == 0:
        loss = re.findall(
            r'Test net output #\d: softmax_loss = (\d*.\d*)', log)
    iteration = [int(i) for i in iteration]
...
#    num_epochs = int(round( (max_iter * iter_size * batch_size*num_gpus) /  1281167. +0.5))
    num_epochs = int(
        round((max_iter * iter_size * batch_size*num_gpus) / 464400. + 0.5))
    ...
    for file in files:
        with open(file, 'r') as fp:
            ...
            net_name = os.path.basename(
                file) if separate else get_net_name(log)
...
def plot_accuracy(top_k, data, value_at_hover=False):
    ...
    for net in nets:
        ...
        iteration, accuracy = (np.array(t)
                               for t in zip(*sorted(zip(iteration, accuracy))))

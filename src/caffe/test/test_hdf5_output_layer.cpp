...
TYPED_TEST(HDF5OutputLayerTest, TestForward)
{
  ...
  hid_t file_id = H5Fopen(this->input_file_name_.c_str(), H5F_ACC_RDONLY,
                          H5P_DEFAULT);
  ASSERT_GE(file_id, 0) << "Failed to open HDF5 file" << this->input_file_name_;
  hdf5_load_nd_dataset(file_id, HDF5_DATA_DATASET_NAME, 0, 4,
                       this->blob_data_);
  hdf5_load_nd_dataset(file_id, HDF5_DATA_LABEL_NAME, 0, 4,
                       this->blob_label_);
  herr_t status = H5Fclose(file_id);
  EXPECT_GE(status, 0) << "Failed to close HDF5 file " << this->input_file_name_;
  ...
  ASSERT_GE(
      file_id, 0)
      << "Failed to open HDF5 file" << this->input_file_name_;
  ...
  EXPECT_GE(status, 0) << "Failed to close HDF5 file " << this->output_file_name_;
}

...
def addResNobn_small(model, name, bottom, num_output, group, j, fix_dim, dilation=False):
    ...
    block, top = addDropout(model=block, name='{}dropout'.format(
        prefix), bottom=top, ratio=0.2)

    if (fix_dim and (j == 1)):
        block, res_top = addConv(model=block, name='{}skipConv'.format(prefix), bottom=bottom, num_output=num_output,
    ...
    block, top = addActivation(
        model=block, name="{}relu".format(prefix), bottom=top)
...
def addResNobn_large(model, name, bottom, num_output, group, j, fix_dim, dilation=False):

    prefix = "{name}.{j}.".format(name=name, j=str(j))

    model, top = addConvRelu(model, name='{}conv1'.format(prefix), bottom=bottom, num_output=num_output,
                             kernel_size=1, group=1, stride=2 if (fix_dim and (j == 1)) else 1, pad=0)
    model, top = addConvRelu(model, name='{}conv2'.format(prefix), bottom=top, num_output=num_output,
                             kernel_size=3, group=group, stride=1, pad=1)
    model, top = addDropout(model, name='{}dropout'.format(
        prefix), bottom=top, ratio=0.5)
    ...
    model, top = addActivation(model, name="{}relu".format(prefix), bottom=top)
...
def addResNobn_SuperBlock(model, bottom, i, num_subBlocks, num_output, group, fix_dim, net_type, dilation=False):
    name = "res{i}".format(i=str(i))
    model = addComment(model, comment=name)
    top = bottom
    for j in xrange(1, num_subBlocks + 1):
        if net_type == "small":
            model, top = addResNobn_small(model, name, bottom=top, num_output=num_output, group=group,
                                          j=j, fix_dim=fix_dim, dilation=dilation)
        else:
            model, top = addResNobn_large(model, name, bottom=top, num_output=num_output, group=group,
                                          j=j, fix_dim=fix_dim, dilation=dilation)
    return model, top
...
def buildResnetNobn(netConfig, name, net_type):
    ...
    for i in xrange(1, num_blocks+1):
        ...
        bottom = top
        model, top = addResNobn_SuperBlock(
            model, bottom, i+1, num_subBlocks, num_output, group, fix_dim, net_type)

    model, top = addPool(model, name="pool2", bottom=top,
                         kernel_size=7, stride=1, pool_type="AVE")
#    model, top = addDropout(model, name="dropout", bottom=top, ratio=0.5)
    model, top = addFC(model, name="fc", bottom=top,
                       num_output=1000, filler='msra')

    fc_top = top
    model, top = addSoftmaxLoss(model, name="loss", bottom_1=fc_top)
    model, top = addAccuracy(
        model, name="accuracy/top-1", bottom_1=fc_top, k=1)
    model, top = addAccuracy(
        model, name="accuracy/top-5", bottom_1=fc_top, k=5)
...
def main():
    ...
    netConfig = numpy.matrix([
        ...
        [256, 2, 1, 1],
        [512, 2, 1, 1]])
    model = buildResnetNobn(netConfig, name="Resnet18_nobn", net_type="small")
    fp = open("resnet18_nobn.prototxt", 'w')
    fp.write(model)
    ...
    netConfig = numpy.matrix([
        ...
        [256, 6, 1, 1],
        [512, 3, 1, 1]])
    model = buildResnetNobn(netConfig, name="Resnet34_nobn", net_type="small")
    fp = open("resnet34_nobn.prototxt", 'w')
    fp.write(model)
    ...
    model = buildResnetNobn(netConfig, name="Resnet50_nobn", net_type="large")
    fp = open("resnet50_nobn.prototxt", 'w')
    fp.write(model)

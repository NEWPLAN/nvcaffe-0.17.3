...
protected:
  GradientBasedSolverTest() : seed_(1701), num_(4), channels_(3), height_(10), width_(10),
                              share_(false)
  {
    ...
    for (int i = 0; i <= D; ++i)
    {
      ...
      for (int j = 0; j <= D; ++j)
      {
        ...
        for (int k = 0; k < N; ++k)
        {
          ...
          const Dtype element_i = (i == D) ? Dtype(1.) : data.template cpu_data<Dtype>()[k * D + i];
          const Dtype element_j = (j == D) ? Dtype(1.) : data.template cpu_data<Dtype>()[k * D + j];
      ...
      grad += weight_decay *
              ((i == D) ? bias.template cpu_data<Dtype>()[0] : weights.template cpu_data<Dtype>()[i]);
      // Finally, compute update.
      const vector<shared_ptr<TBlob<Dtype>>> &history = solver_->history();
      if (solver_->type() != string("AdaDelta") && solver_->type() != string("Adam"))
      {
      ...
      const Dtype history_value = (i == D) ? history[1]->cpu_data()[0] : history[0]->cpu_data()[i];
      ...
      {
        const Dtype update_history_value = (i == D) ? history[1 + num_param_blobs]->cpu_data()[0] : history[0 + num_param_blobs]->cpu_data()[i];
        const Dtype weighted_gradient_average =
            momentum * history_value + (1. - momentum) * (grad * grad);
        update_value = grad * std::sqrt((update_history_value + delta_) / (weighted_gradient_average + delta_)) * learning_rate;
      ...
      else if (solver_->type() == string("Adam"))
      {
        ...
        const Dtype v = (i == D) ? history[1 + num_param_blobs]->cpu_data()[0] : history[0 + num_param_blobs]->cpu_data()[i];

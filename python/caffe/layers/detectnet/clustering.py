    ...
    * top [i] - list of groundtruth bbox for each class
    ...
    def setup(self, bottom, top):
        ...
        if len(top) != self.num_classes:
            raise ValueError("Unexpected number of tops: %d != %d" %
                             (len(top), self.num_classes))

    def reshape(self, bottom, top):
        n_images = bottom[0].data.shape[0]
        num_classes = bottom[0].data.shape[1]
        if num_classes != self.num_classes:
            raise ValueError("Unexpected number of classes: %d != %d, bottom[0] shape=%s" % (
                num_classes, self.num_classes, repr(bottom[0].data.shape)))
...
def gridbox_to_boxes(net_cvg, net_boxes, self):
    ...
    x1 = (np.asarray([net_boxes[0][y[i]][x[i]]
                      for i in list(range(x.size))]) + mx)
    y1 = (np.asarray([net_boxes[1][y[i]][x[i]]
                      for i in list(range(x.size))]) + my)
    x2 = (np.asarray([net_boxes[2][y[i]][x[i]]
                      for i in list(range(x.size))]) + mx)
    y2 = (np.asarray([net_boxes[3][y[i]][x[i]]
                      for i in list(range(x.size))]) + my)
...
def vote_boxes(propose_boxes, propose_cvgs, mask, self):
    ...
    nboxes, weights = cv.groupRectangles(
        [[e[0], e[1], e[2]-e[0], e[3]-e[1]]
            for e in np.array(propose_boxes).tolist()],
    ...
    if len(nboxes):
        for rect, weight in zip(nboxes, weights):
            if rect[3] >= self.min_height:
                ...
                detection = [rect[0], rect[1], rect[0] +
                             rect[2], rect[1]+rect[3], confidence]
...
def cluster(self, net_cvg, net_boxes):
    ...
    for i in range(batch_size):
        ...
        if (self.is_groundtruth):
            ...
            boxes_cur_image = vote_boxes(
                propose_boxes, propose_cvgs, mask, self)

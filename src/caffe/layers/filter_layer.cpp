                                        ...
                                        const vector<Blob *> &top)
{
  ...
  {
    CHECK_EQ(bottom[selector_index]->shape(0), bottom[i]->shape(0)) << "Each bottom should have the same 0th dimension as the selector blob";
  }
    ...
    if (propagate_down[i])
    {
      ...
      for (int n = 0; n < bottom[i]->shape(0); n++)
      {
        ...
        else
        {
          ...
          if (n != batch_offset)
          { // this data was not been forwarded
            caffe_set(dim, Btype(0),
                      bottom[i]->mutable_cpu_diff<Btype>() + data_offset_bottom);
          }
          else
          { // this data was been forwarded

...
def addResSelu_small(model, name, bottom, num_output, group, j, fix_dim, dilation=False):
...
def addResSelu_SuperBlock(model, bottom, i, num_subBlocks, num_output, group, fix_dim, net_type, dilation=False):
    name = "res{i}".format(i=str(i))
    model = addComment(model, comment=name)
    top = bottom
    for j in xrange(1, num_subBlocks + 1):
        if net_type == "small":
            model, top = addResSelu_small(model, name, bottom=top, num_output=num_output, group=group,
...
def buildResnetSelu(netConfig, name, net_type):
    ...
    for i in xrange(1, num_blocks+1):
        ...
        bottom = top
        model, top = addResSelu_SuperBlock(
...
def main():
    ...
    netConfig = numpy.matrix([
        ...
        [256, 2, 1, 1],
        [512, 2, 1, 1]])
    model = buildResnetSelu(netConfig, name="Resnet18_selu", net_type="small")
    fp = open("resnet18_selu.prototxt", 'w')
    fp.write(model)
    ...
    netConfig = numpy.matrix([
        ...
        [256, 6, 1, 1],
        [512, 3, 1, 1]])
    model = buildResnetSelu(netConfig, name="Resnet34_selu", net_type="small")
    fp = open("resnet34_selu.prototxt", 'w')
    fp.write(model)

                                                  ...
                                                  const vector<Blob *> &top)
{
  ...
  if (this->rank_ % Caffe::device_in_use_per_host_count() == 0)
  {
    ...
    if (image_data_param.dataset_share_per_node() > 1.F && this->phase_ == TRAIN)
    {
      lines_[id_].resize(std::lround(lines_[id_].size() / image_data_param.dataset_share_per_node()));
    }
  }
  LOG_IF(INFO, P2PManager::global_rank() == 0)
    ...
    if (line_ids_[thread_id] >= lines_size)
    {
          ...
          this->rank_ % Caffe::device_in_use_per_host_count() == 0)
      {
        if (this->phase_ == TRAIN)
        {
          ...
          if (epoch_count_ == 0UL)
          {
            epoch_count_ += std::lround(lines_[id_].size() * image_data_param.dataset_share_per_node());

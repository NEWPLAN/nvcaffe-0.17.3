    ...
    const vector<Blob *> &bottom, const vector<Blob *> &top)
{
  ...
  {
    fwd_algo_[i] = (cudnnConvolutionFwdAlgo_t)(user_algos_override_[0] >= 0 ? user_algos_override_[0] : (use_dilation ? CUDNN_CONVOLUTION_FWD_ALGO_IMPLICIT_GEMM : CUDNN_CONVOLUTION_FWD_ALGO_IMPLICIT_PRECOMP_GEMM));
    bwd_data_algo_[i] = (cudnnConvolutionBwdDataAlgo_t)(user_algos_override_[1] >= 0 ? user_algos_override_[1] : CUDNN_CONVOLUTION_BWD_DATA_ALGO_1);
    bwd_filter_algo_[i] = (cudnnConvolutionBwdFilterAlgo_t)(user_algos_override_[2] >= 0 ? user_algos_override_[2] : CUDNN_CONVOLUTION_BWD_FILTER_ALGO_1);
...
size_t CuDNNConvolutionLayer<Ftype, Btype>::AllocateFindExWorkspace()
{
  ...
  // 2+ pages => reallocate
  size_t req_bytes = align_down<8>(bytes_available > 2UL * PAGE_SIZE ? bytes_available - 2UL * PAGE_SIZE : 0UL);

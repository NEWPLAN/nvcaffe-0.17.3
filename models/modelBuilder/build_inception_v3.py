...
def addInceptionT3(model, name, bottom, p1_num_out, p2_num_out, p3_proj, p3_num_out, p4_num_out, pool):
    ...
    p2_tops = [top2a, top2b]
    model, top2 = addConcat(
        model, name='{}/p2_concat'.format(name), bottoms=p2_tops)
    ...
    model, top3b = addConvBnRelu(model, name='{}/p3_3x1'.format(name), bottom=top3, num_output=p3_num_out,
                                 kernel_h=3, kernel_w=1, pad_h=1, pad_w=0, stride=1, filler="xavier")
    p3_tops = [top3a, top3b]
    model, top3 = addConcat(
        model, name='{}/p3_concat'.format(name), bottoms=p3_tops)
...
def addAuxLoss(model, name, bottom):
    ...
    model, top = addFC(model, name="{}/fc1".format(name),
                       bottom=top, num_output=1024, filler='xavier')
    model, top = addActivation(model, name="{}/relu".format(name), bottom=top)
    model, top = addFC(model, name="{}/fc2".format(name),
                       bottom=top, num_output=1000, filler='xavier')
    fc_top = top
    model, top = addSoftmaxLoss(
        model, name="{}/loss".format(name), bottom_1=fc_top, loss_weight=0.3)
    model, top = addAccuracy(
        model, name="{}/top-1".format(name),  bottom_1=fc_top, k=1)
    model, top = addAccuracy(
        model, name="{}/top-5".format(name), bottom_1=fc_top, k=5)
...
def buildInception_v3():
    ...
    model, data_top = addData(
        model, train_batch, test_batch, train_file, test_file, mean_file, crop_size)
    ...
    model, top = addConvBnRelu(model, 'conv3', top, num_output=64,
                               kernel_size=3, stride=1, pad=1, filler="xavier")
    # 147 x 147 x 64
    model, top = addPool(model, 'pool1', top, kernel_size=3,
                         stride=2, pad=0, pool_type='MAX')
    # 73 x 73 x 64
    ...
    model, top = addInceptionT1(
        model, "3A", top, 64, 64, 96, 48, 64, 64, 'AVE')
    model, top = addInceptionT1(
        model, "3B", top, 64, 64, 96, 48, 64, 64, 'AVE')
    model, top = addInceptionT1(
        model, "3C", top, 64, 64, 96, 48, 64, 64, 'AVE')
    ...
    model, top = addInceptionT3(
        model, '5A', top, 320, 384, 448, 384, 192, 'AVE')
    model, top = addInceptionT3(
        model, '5B', top, 320, 384, 448, 384, 192, 'AVE')

    # Loss3---------------------------------------------------------------------
    model, top = addPool(model, name="loss/pool", bottom=top,
                         kernel_size=7, stride=1, pool_type="AVE")
#    model, top = addDropoutl(model, name="loss/dropout", bottom =top, ratio=0.5)
    model, top = addFC(model, name="loss/fc", bottom=top,
                       num_output=1000, filler='xavier')
    fc_top = top

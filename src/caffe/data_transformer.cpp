...
void DataTransformer<Dtype>::image_random_resize(const cv::Mat &src, cv::Mat &dst) const
{
  ...
  const float scale = img_width >= img_height ? static_cast<float>(new_size) / static_cast<float>(img_height) : static_cast<float>(new_size) / static_cast<float>(img_width);
  ...
  else
  {
    cv::resize(
        ...
        0., 0.,
        new_height <= img_height && new_width <= img_width ? (int)param_.interpolation_algo_down() : (int)param_.interpolation_algo_up());
  }
}
...
void DataTransformer<Dtype>::InitRand()
{
  ...
  const uint64_t random_seed = param_.random_seed() >= 0 ? static_cast<uint64_t>(param_.random_seed()) : Caffe::next_seed();

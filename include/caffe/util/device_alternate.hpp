...
#define CUDA_CHECK_ARG(condition, arg)                                                      \
  ...
  {                                                                                         \
    cudaError_t error = condition;                                                          \
    CHECK_EQ(error, cudaSuccess) << " " << cudaGetErrorString(error) << " (" << arg << ")"; \
  } while (0)
...
#define CUDA_CHECK_ARG2(condition, arg1, arg2)                                                                \
  ...
  {                                                                                                           \
    cudaError_t error = condition;                                                                            \
    CHECK_EQ(error, cudaSuccess) << " " << cudaGetErrorString(error) << " (" << arg1 << ") (" << arg2 << ")"; \
  } while (0)
...
#define CURAND_CHECK_ARG(condition, arg)                                                                  \
  ...
  {                                                                                                       \
    curandStatus_t status = condition;                                                                    \
    CHECK_EQ(status, CURAND_STATUS_SUCCESS) << " "                                                        \
                                            << caffe::curandGetErrorString(status) << " (" << arg << ")"; \
  } while (0)
